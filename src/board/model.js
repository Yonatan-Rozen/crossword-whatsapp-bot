...
// Shared crossword grid model. Originally a fixed single grid (ported from
// crossword_board.py); now a `buildBoard(boardRows)` factory so any dynamically solved
// layout (see src/board/layoutSolver.js) can be numbered/queried the same way.
//
// Board grid numbering is RTL: an across entry is numbered at its RIGHTMOST cell and its
// span extends leftward (decreasing column index). Down entries number at the top and
// extend downward as usual.

function buildBoard(boardRows) {
  const ROWS = boardRows.length;
  const COLS = boardRows[0].length;

  function isBlack(row, col) {
    return boardRows[row][col] === "#";
  }
